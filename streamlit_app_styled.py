...
def load_data(path: str):
    """
    Load an Excel file from a local path or a direct/RAW GitHub URL.
    """
    try:
        # For remote GitHub RAW URL or other http(s) location pandas can read directly.
...
# <-- Put the RAW GitHub URL here (works if the repo/file is public) -->
DATA_PATH = "https://raw.githubusercontent.com/tuyishimeandrew/South-Sudan-Dev-t-Analysis/main/SS%20Raw%20Data.xlsx"

# If you prefer to use a local copy, uncomment and adjust the line below:
...
def set_bg(image_file):
    try:
        ...
        # If background image not present locally, skip silently
        pass

# If you have a local background file, keep this; otherwise it's optional
...
if df.empty:
    ...
    if 'Donor' in df_f.columns and 'Budget ($)' in df_f.columns:
        fig = px.bar(
            df_f.groupby('Donor', as_index=False)['Budget ($)'].sum().sort_values('Budget ($)', ascending=False),
